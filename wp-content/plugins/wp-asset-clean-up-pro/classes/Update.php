...
HTML;
    ...
    public function frontendUpdate()
    {
        ...
        \WpAssetCleanUpPro\Admin\UpdatePro::updateAssetsPositions();
        \WpAssetCleanUpPro\Admin\UpdatePro::updateHandleMediaQueriesLoad();
	    // [/wpacu_pro]
        ...
        \WpAssetCleanUpPro\Admin\UpdatePro::storeHardcodedAssetsInfo();
	    // [/wpacu_pro]
    }
...
SQL;
    ...
    public function savePost($postId, $post = '')
    {
        ...
        // [wpacu_pro]
        \WpAssetCleanUpPro\Admin\UpdatePro::updateAssetsPositions(); // Any positions changed?
        \WpAssetCleanUpPro\Admin\UpdatePro::updateHandleMediaQueriesLoad();
        \WpAssetCleanUpPro\Admin\UpdatePro::updateTaxonomyValuesAssocToPostType($post->post_type);
        \WpAssetCleanUpPro\Admin\UpdatePro::updateTaxonomyValuesAssocToPostTypeLoadExceptions($post->post_type);
		// [/wpacu_pro]
		...
		foreach ($assetList as $assetType => $assetDataHandleList) {
			...
			foreach ($assetDataHandleList as $assetObj) {
				...
				if (isset($assetArray['output'])) { // hardcoded assets have an 'output' value
					...
					if ( isset($existingList[$assetType][$globalKey][$assetHandle]['output'], $existingList[$assetType][$globalKey][$assetHandle]['output_min']) &&
                        $existingList[$assetType][$globalKey][$assetHandle]['output'] &&
                        $existingList[$assetType][$globalKey][$assetHandle]['output_min'] ) {
					...
					if ( ! isset( $assetArray['output_min'] ) ) {
						...
						if (strncmp($assetHandle, 'wpacu_hardcoded_script_', 23) === 0 ) {
						...
						} elseif ( (strncmp($assetHandle, 'wpacu_hardcoded_link_', 21) === 0 )
                                   || (strncmp($assetHandle, 'wpacu_hardcoded_style_', 22) === 0 ) ) {
                        ...
                        if ($assetArray['output_min'] === '') {
                            unset($assetArray['output_min']);
                        }
					}
				}
				// [/wpacu_pro]
